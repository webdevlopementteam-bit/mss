...
export default function UserDashboardLayout() {
  ...
  const NavBtn = ({ to, label, icon, end }) => (
    <NavLink to={to} end={end} onClick={() => setSide(false)}
      style={({ isActive }) => ({ display: "flex", alignItems: "center", gap: 10, width: "100%",
        padding: "11px 14px", borderRadius: 10, border: "none", cursor: "pointer",
        background: isActive ? C.redLight : "transparent",
        color: isActive ? C.red : C.muted, textDecoration: "none",
        fontWeight: isActive ? 700 : 500, fontSize: 14, transition: "background .12s",
        marginBottom: 2 })}>
      {({ isActive }) => (
        <>
  ...
  return (
    <div style={{ fontFamily: "'Inter','Segoe UI',system-ui,sans-serif", background: C.bg,
      ...
      {/* Sidebar */}
      <aside style={{ width: 220, background: C.white, borderRight: `1px solid ${C.border}`,
        display: "flex", flexDirection: "column", padding: "18px 12px 20px",
        position: "sticky", top: 0, alignSelf: "flex-start", height: "100vh",
        flexShrink: 0,
        transition: "transform .22s" }} className={`sidebar-el${sideOpen ? " open" : ""}`}>

        {/* Avatar */}
        <div style={{ background: C.redLight, borderRadius: 12, padding: "12px",
          display: "flex", alignItems: "center", gap: 10, marginBottom: 18, marginTop: 4 }}>
          <div style={{ width: 38, height: 38, borderRadius: "50%", background: C.red, flexShrink: 0,
        ...
        <div style={{ flex: 1 }} />
        <button onClick={handleSignOut} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%",
          padding: "11px 14px", borderRadius: 10, border: "none", cursor: "pointer",
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}
        ...
        {/* Mobile bar */}
        <div className="mobile-bar" style={{ display: "none", background: C.white,
