...
const StepHeader = ({ number, title, subtitle }) => (
  <div className="flex items-start gap-3 mb-5">
    ...
    <div>
      <h2 className="text-sm sm:text-base font-bold text-gray-900 tracking-tight">{title}</h2>
...
const Checkout = () => {
  ...
  const subtotal = useMemo(
    () => cart.reduce((acc, item) => acc + getLineUnitPrice(item) * item.quantity, 0),
    [cart]
  );
  ...
  const gst = useMemo(
    () =>
      cart.reduce(
        (acc, item) =>
          acc + (getLineUnitPrice(item) * item.quantity * (Number(item.gst) || 0)) / 100,
        0
      ),
    [cart]
  );
  ...
  const validateForm = () => {
    ...
    for (const [condition, message] of checks) {
      if (condition) { toast.error(message); return false; }
    }
    return true;
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-16">
        <div className="text-center max-w-xs mx-auto">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-2">Nothing to checkout</h2>
          <p className="text-gray-500 text-sm mb-6">Add some products to your cart first.</p>
          <Link to="/shop" className="inline-block bg-primaryColor text-white px-8 py-3 rounded-xl font-semibold hover:opacity-90 transition-all shadow-md shadow-primaryColor/20">
            Browse Products
          </Link>
        </div>
      </div>
    ...
    <section className="bg-[#F8F9FB] min-h-screen py-8 sm:py-10">
      <div className="px-4 sm:px-6 lg:px-side">
        ...
        <div className="mb-6 sm:mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">Secure Checkout</h1>
        ...
        <div className="grid lg:grid-cols-3 gap-5 lg:gap-7 items-start">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-gray-100">
              <StepHeader number="1" title="Customer Information" subtitle="We'll use this to confirm and update you on your order" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Full Name</label>
                  <input type="text" name="fullName" placeholder="e.g. Riya Sharma"
                    value={formData.fullName} onChange={handleChange} className={inputClass} />
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Mobile Number</label>
            ...
            <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-gray-100">
              <StepHeader number="2" title="Delivery Address" subtitle="Where should we send your order?" />
              <div className="space-y-3">
                <div className="space-y-1">
                  <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Street Address</label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <input type="text" name="pincode" placeholder="6-digit pincode" maxLength={6}
                      value={formData.pincode} onChange={handleChange} className={inputClass} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Landmark <span className="normal-case font-normal text-gray-400">(optional)</span></label>
                    <input type="text" name="landmark" placeholder="Near school, temple, etc."
                      value={formData.landmark} onChange={handleChange} className={inputClass} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-gray-100">
              <StepHeader
                ...
                title={
                  <span className="flex items-center gap-2">
                    Upload Prescription
                    <span className="text-[10px] font-bold text-white bg-red-500 rounded-md px-1.5 py-0.5 tracking-wide">REQUIRED</span>
                  </span>
                }
              ...
              {!prescription ? (
                ...
                >
                  ...
                  />
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors duration-200 ${dragOver ? "bg-primaryColor/10" : "bg-white border border-gray-200"}`}>
                    <svg className={`w-6 h-6 transition-colors ${dragOver ? "text-primaryColor" : "text-gray-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-green-800 truncate">{prescription.name}</p>
                    <p className="text-xs text-green-600 mt-0.5">{(prescription.size / 1024).toFixed(0)} KB · Prescription uploaded</p>
                  </div>
                  <button
              ...
              <p className="mt-3 text-[11px] text-gray-400 flex items-start gap-1.5">
                ...
                Your prescription is stored securely and only shared with our licensed pharmacists for verification.
              </p>
            </div>

            {/* Step 4 — Payment */}
            <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-gray-100">
              <StepHeader number="4" title="Payment Method" subtitle="Choose how you'd like to pay" />
          ...
          <div className="w-full">
            <div className="bg-white rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-gray-100 lg:sticky lg:top-6 overflow-hidden">
              ...
              <div className="px-5 sm:px-6 pt-5 pb-4 border-b border-gray-100">
                <h2 className="text-sm font-bold text-gray-900 tracking-tight">Order Summary</h2>
                <p className="text-xs text-gray-400 mt-0.5">{itemCount} {itemCount === 1 ? "item" : "items"} in your cart</p>
              </div>
              ...
              <div className="px-5 sm:px-6 py-4 space-y-3.5 max-h-64 overflow-y-auto">
                {cart.map((item) => {
                  ...
                  return (
                    <div key={getCartLineId(item, item.variant)} className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-xl bg-[#F5F2EE] flex-shrink-0 overflow-hidden flex items-center justify-center border border-gray-100">
                        {image ? (
                          ...
                          <svg className="w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.3}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {item.variant?.combination && (
                          <p className="text-[11px] text-gray-400">{item.variant.combination}</p>
                        )}
                        <p className="text-[11px] text-gray-400 mt-0.5">
                          Qty {item.quantity}
                          {item.quantity > 1 && (
                            <span className="ml-1">· ₹{unitPrice.toLocaleString("en-IN")} each</span>
                          )}
                        </p>
                      </div>
              ...
              <div className="px-5 sm:px-6 pb-5 border-t border-gray-100 pt-4 space-y-2.5 text-sm">
                ...
                <div className="flex justify-between text-gray-500 text-xs">
                  ...
                  <span className="text-gray-700 font-medium">
                    {gst > 0 ? `₹${gst.toLocaleString("en-IN", { maximumFractionDigits: 2 })}` : "₹0"}
                  </span>
                </div>
                <div className="border-t border-gray-100 pt-3 flex justify-between font-bold">
                  <span className="text-gray-900 text-sm">Total</span>
                  <span className="text-primaryColor text-base">₹{total.toLocaleString("en-IN")}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-px border-t border-gray-100 bg-gray-100">
                {[
                  ...
                  <div key={text} className="flex items-center gap-2 bg-white px-3 py-2.5">
