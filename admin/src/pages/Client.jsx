...
const Client = () => {
  ...
  const fetchClients = async (pageNum = 1, searchQuery = "") => {
    try {
      const res = await API.get(`/client?page=${pageNum}&limit=5&search=${searchQuery}`);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editID) {
        await API.put(`/client/${editID}`, formData);
      } else {
 ...
 const handleDelete = async (id) => {
    try {
      ...
      const res = await API.get(
        `/client?page=${page}&limit=5&search=${search}&t=${Date.now()}`,
      );
  ...
  const handleEdit = (client) => {
    setForm({
      name: client.name,
      image: null,
      imagePreview: `${IMG_URL}/${client.image}`,
    });
  ...
  return (
    <div className="p-6 text-on-surface">
        ...
        <div className="flex justify-between items-center mb-6 mt-10">
          <h1 className="text-2xl font-semibold text-white/70">Client</h1>
          ...
          <div className="flex gap-3">
            <button
              onClick={async () => {
                ...
                try {
                  await Promise.all(
                    selected.map((id) => API.delete(`/client/${id}`)),
                  );
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-white/10">
              <tr>
                ...
                <th className="p-4 text-left text-white/80">Client Name</th>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-end z-50">
            <div className="w-[520px] bg-[#0f131c] border-l border-white/10 h-full p-6 shadow-2xl overflow-y-auto">
              ...
              <div className="space-y-5">
                {/* NAME */}
                <div>
                  <label htmlFor="Brand Name" className="text-white/80 text-sm">Client Name</label>
                <input
