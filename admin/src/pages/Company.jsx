...
const Company = () => {
  ...
  const fetchCompanies = async (pageNum = 1, searchQuery = "") => {
    try {
      const res = await API.get(`/company?page=${pageNum}&limit=5&search=${searchQuery}`);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editID) {
        await API.put(`/company/${editID}`, formData);
      } else {
 ...
 const handleDelete = async (id) => {
    try {
      ...
      const res = await API.get(
        `/company?page=${page}&limit=5&search=${search}&t=${Date.now()}`,
      );
  ...
  const handleEdit = (company) => {
    setForm({
      name: company.name,
      image: null,
      imagePreview: `${IMG_URL}/${company.image}`,
    });
  ...
  return (
    <div className="p-6 text-on-surface">
        ...
        <div className="flex justify-between items-center mb-6 mt-10">
          <h1 className="text-2xl font-semibold text-white/70">Company</h1>
          ...
          <div className="flex gap-3">
            <button
              onClick={async () => {
                ...
                if (!confirm("Delete selected companies?")) return;

                try {
                  await Promise.all(
                    selected.map((id) => API.delete(`/company/${id}`)),
                  );
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-white/10">
              <tr>
                ...
                <th className="p-4 text-left text-white/80">Company Name</th>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-end z-50">
            <div className="w-[520px] bg-[#0f131c] border-l border-white/10 h-full p-6 shadow-2xl overflow-y-auto">
              ...
              <div className="space-y-5">
                {/* NAME */}
                <div>
                  <label htmlFor="Brand Name" className="text-white/80 text-sm">Company Name</label>
                <input
