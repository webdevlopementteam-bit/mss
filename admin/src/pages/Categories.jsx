...
const Category = () => {
  ...
  const fetchCategories = async (pageNum = 1, searchQuery = "") => {
    try {
      const res = await API.get(
        `/category?page=${pageNum}&limit=5&search=${searchQuery}`,
      );
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editId) {
        await API.put(`/category/${editId}`, formData);
      } else {
  ...
  const handleEdit = (cat) => {
    setForm({
      ...
      image: null,
      imagePreview: `${IMG_URL}/${cat.image}`,
  ...
  return (
    <div className="p-6 text-on-surface">
      ...
      <div className="flex justify-between items-center mb-6 mt-10">
        <h1 className="text-2xl font-semibold text-white/70">Category</h1>
        <div className="flex gap-3">
          <button
            onClick={async () => {
              ...
              try {
                await Promise.all(
                  selected.map((id) => API.delete(`/category/${id}`)),
                );

                if (categories.length === selected.length && page > 1) {
                  setPage((prev) => prev - 1);
                } else {
                  fetchCategories(page, search);
                }
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-white/10">
            <tr>
              ...
              <th className="p-4 text-left text-white/80">Parent</th>
            ...
            <div className="space-y-5">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="w-full p-3 mt-2 rounded-lg bg-[#1a202b] border border-white/10 text-white flex justify-between items-center"
                >
                  {form.parentCategory
                ...
                {openDropdown && (
                  <div className="absolute w-full mt-2 bg-[#1a202b] border border-white/10 rounded-lg shadow-lg z-50 max-h-40 overflow-y-auto">
                    <div
                      onClick={() => {
                        setForm({ ...form, parentCategory: "" });
                    ...
                    {categories.map((c) => (
                      <div
                        key={c._id}
                        onClick={() => {
                          setForm({ ...form, parentCategory: c._id });
                          setOpenDropdown(false);
                        }}
                        className="px-4 py-2 hover:bg-white/10 cursor-pointer text-white"
                      >
