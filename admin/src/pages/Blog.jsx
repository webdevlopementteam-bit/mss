...
const Blog = () => {
  ...
  const fetchBlogs = async (pageNum = 1, searchQuery = "") => {
    try {
      const res = await API.get(
        `/blog?page=${pageNum}&limit=5&search=${searchQuery}`,
      );
  ...
  //   const filteredData = blogs.filter((c) =>
  //     c.name.toLowerCase().includes(search.toLowerCase()),
  //   );

  //   setPage(1);
  //   setFiltered(filteredData); // ✅ fixed
  // };

  const handleReset = () => {
    setSearch("");
    setPage(1);
    setFiltered(blogs); // ✅ fixed
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      // 🔥 fresh fetch
      const res = await API.get(
        `/blog?page=${page}&limit=5&search=${search}&t=${Date.now()}`,
      );
    ...
    <>
      <div className="p-6 text-on-surface">
        ...
        <div className="flex justify-between items-center mb-6 mt-10">
          <h1 className="text-2xl font-semibold text-white/70">Blog</h1>
          ...
          <div className="flex gap-3">
            <button
              onClick={async () => {
                ...
                if (!confirm("Delete selected blogs?")) return;

                try {
                  await Promise.all(
                    selected.map((id) => API.delete(`/blog/${id}`)),
                  );
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-white/10">
              <tr>
                ...
                <th className="p-4 text-left text-white/80">Blog</th>
                      ...
                      <div className="flex justify-center items-center">
                        <button
                          ...
                          className={`w-12 h-6 flex items-center rounded-full p-1 transition ${
                            a.isPublished ? "bg-green-500" : "bg-gray-600"
                          }`}
                        >
                          <div
                            className={`bg-white w-4 h-4 rounded-full shadow-md transform transition ${
                              a.isPublished ? "translate-x-6" : ""
                            }`}
                          />
                        </button>
                      </div>
